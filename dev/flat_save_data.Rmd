...
#' 
#' @param url Option: Persistent url to the data
#' @importFrom readxl excel_sheets read_excel
#' @importFrom utils download.file
#' @importFrom dplyr mutate rename select
#' @importFrom stringr str_trim
#' @importFrom janitor clear_names
...
#' @details Removes uneeded rows, renamss localities to make their names consistent across years and converts columns with prices to numeric columns.
...
clean_raw_data <- function(raw_data){
  raw_data %>%
    mutate(locality = 
             case_when (
              grepl("Luxembourg-Ville", locality) ~ "Luxembourg",
              grepl("P.tange", locality) ~ "Pétange",
              TRUE ~ locality
             )
    ) %>%
...
#' @param table_position Optional: Scraping returns a list of tables, so users need to specify tje correct table. Defaults to 3, the position of the table as of writing. 
...
#' get)current_communes Downloads the list of current communes from Wikipedia 
#' 
#' @param url Optional: Persistent url to the data
...
#' get_test_communes Creates a list of communes that shoiuld be in the data
#' @param former_communes Former communes df returned by get_former_communes()
#' @param current_communes Current communes of as returned by get_current_communes()
#' @return A data frame
#' @export 
...
testthat::expect_true(all(communes %in% unique(flat_data$locality)))
